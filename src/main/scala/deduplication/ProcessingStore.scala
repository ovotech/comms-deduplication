...
object ProcessingStore {
  ...
  def fromDynamo[F[_]: Async, ID, ProcessorID](config: Config[ProcessorID])(
    ...
    fromDynamo(config, dynamoDbR)
  }

  def fromDynamo[F[_]: Async, ID, ProcessorID](
      config: Config[ProcessorID],
      dynamoDbR: Resource[F, AmazonDynamoDBAsync])(
      implicit idDf: DynamoFormat[ID],
      processorIdDf: DynamoFormat[ProcessorID],
      timer: Timer[F],
      ec: ExecutionContext
